...
sum666 = on_fullmatch(plugin_config.sum666_keyword, rule=checker & isInService(
    "超级666", 1) if isInService else True, priority=50)
...
async def sum666_handle(bot: Bot, matcher: Matcher, event: Event):
    ...
    if key in Data.keys():
        ...
        Data[key] = 1
    File = open(JSON_PATH, 'w+', parents=True, exist_ok=True)
